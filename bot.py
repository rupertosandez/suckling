...
_MACGUFFIN_CLAIM_LOCKS: dict[str, asyncio.Lock] = {}


def _merge_macguffin_inventory(records: list[dict]) -> list[dict]:
...
def _find_macguffin_matches(query: str, cards: list[dict]) -> list[dict]:
...
def _macguffin_claim_lock(user_id: str) -> asyncio.Lock:
    lock = _MACGUFFIN_CLAIM_LOCKS.get(user_id)
    if lock is None:
        lock = asyncio.Lock()
        _MACGUFFIN_CLAIM_LOCKS[user_id] = lock
    return lock


async def _announce_macguffin_drop(
...
@bot.tree.command(name="claimguffin", description="claim your one free macguffin")
async def claimguffin(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True)

    user_id = str(interaction.user.id)
    async with _macguffin_claim_lock(user_id):
        ...
        try:
            ...
            await _announce_macguffin_drop(interaction, card, interaction.user)
...
@bot.tree.command(name="giftguffin", description="gift one of your macguffins to another member")
@app_commands.describe(
...
async def giftguffin(
    ...
    owned_cards = _merge_macguffin_inventory(records)
    matches = _find_macguffin_matches(card, owned_cards)
...
@bot.tree.command(name="myguffins", description="view your macguffin collection")
async def myguffins(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True)
    ...
    cards = _merge_macguffin_inventory(records)
    if not cards:
        await interaction.followup.send(
...
async def adminguffins(
    ...
    inventory = _merge_macguffin_inventory(records)
    ...
    if action.value == "random":
        ...
        await _announce_macguffin_drop(interaction, selected, user)
        return
    ...
    if action.value == "remove":
        matches = _find_macguffin_matches(card, inventory)
        if not matches:
            await interaction.followup.send(
    ...
    if action.value == "add":
        matches = _find_macguffin_matches(
            card,
