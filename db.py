    ...
    with _connect() as conn:
        try:
            conn.execute(
                ...
                "VALUES (?, ?, ?, ?)",
                (tmdb_id, title, added_by, datetime.utcnow().isoformat()),
            )
            return True
...
def record_provider(tmdb_id: int, provider_name: str) -> None:
    with _connect() as conn:
        conn.execute(
            ...
            "VALUES (?, ?, ?)",
            (tmdb_id, provider_name, datetime.utcnow().isoformat()),
        )
...
def record_daily_rec(tmdb_id: int, title: str) -> None:
    with _connect() as conn:
        conn.execute(
            ...
            (tmdb_id, title, datetime.utcnow().isoformat()),
        )


def recent_rec_ids(within_days: int = 30) -> set[int]:
    cutoff = datetime.utcnow().timestamp() - (within_days * 86400)
    with _connect() as conn:
...
def increment_guess_score(user_id: str, user_tag: str, points: int = 1) -> int:
    with _connect() as conn:
        conn.execute(
            ...
            (user_id, user_tag, points, datetime.utcnow().isoformat(), points),
        )
        row = conn.execute(
